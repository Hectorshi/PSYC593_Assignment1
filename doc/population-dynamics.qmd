...
Load tidyverse and dataset; hide the result from the output
```{r, message = FALSE}
# Load packages ----
library(tidyverse)
# Hide messages from loading package
```

```{r}
# Read data, modified to fit the new directory ----
...
# Change swedenData to sweden_data for tidy version
```
...
Two ways of creating new variable:
1) creating individually
```{r}
...
```{r}
# Function to compute the Crude Birth Rate (CBR), changed populationData to population_data
...
The total person-years data for the 3 different datasets are stored in kenyacbr,
swedencbr and worldcbr.
...
The pattern shows that the the fertility rate for women of the reproductive age
is greater in 1950-1955 than 2005 - 2010 for both countries, where Kenya drops more significantly.
Kenya has a greater fertility rate for women of the reproductive age than 
Sweden for both of theperiods. In Kenya, highest ASFR is calculated 
for age 20-24 for both era. 
In Sweden, highest ASFR is calculated for age 20-24 in 1950-1955, age 30-34 in
2005-2010.
...
```
In general, total number of women increased by 2.53 from 1950 to 2005;
total number of birth increased by 1.38 from 1950 to 2005.
...
The crude death rate for world and Kenya decreased where increased for Sweden.
...
Even the CDR is very close for Kenya and Sweden, Sweden has a significantly 
lower ASDR than Kenya in 2005 - 2010.
...
mutate(kenya_data,
 temp_cdr  = asdr * sweden_data$pop_por) %>%
 group_by(period) %>%
 summarise(cdrresweden = sum(temp_cdr))
```

Kenya's CDR with Sweden's population distribution is very similar to its own
in 1950, but more than twice of its own in 2005. It might be caused by the
change of population distribution due to the population bomb between 1950-2005.